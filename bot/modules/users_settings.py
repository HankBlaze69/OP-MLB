...
def get_user_settings(from_user, key=None):
    ...
    uplan = "Paid 💸" if is_paid(user_id) else "Free ✨"
    if key is None:
        ...
        buttons.sbutton("❌", f"userset {user_id} close")
        text = "User Settings:"
        button = buttons.build_menu(1)
    elif key == 'universal':
        imdb = user_dict['imdb_temp'] if user_dict and user_dict.get('imdb_temp') else "✘"
        anilist = user_dict['ani_temp'] if user_dict and user_dict.get('ani_temp') else "✘"
        ytq = user_dict['yt_ql'] if user_dict and user_dict.get('yt_ql') else config_dict['YT_DLP_QUALITY'] if config_dict['YT_DLP_QUALITY'] else "✘"
        ...
        buttxt = "Change/Delete YT-DLP Quality" if ytq != "✘" else "Set YT-DLP Quality"
        ...
        buttons.sbutton("🔙", f"userset {user_id} mback", 'footer')
        buttons.sbutton("❌", f"userset {user_id} close", 'footer')
        button = buttons.build_menu(2)
        text = f'''<u>Universal Settings for <a href='tg://user?id={user_id}'>{name}</a></u>

╭ <b>YT-DLP Quality :</b> {escape(ytq)}
├ <b>Daily Tasks :</b> {dailytas} / {dailytl} per day
├ <b>Last Bot Used :</b> {lastused}
├ <b>User Bot PM :</b> {ubotpm}
├ <b>List Type :</b> {ulist}
├ <b>IMDB :</b> {imdbval if imdbval else imdb}
├ <b>AniList :</b> {anival if anival else anilist}
'''
    elif key == 'mirror':
        prefix = user_dict['mprefix'] if user_dict and user_dict.get('mprefix') else "✘"
        suffix = user_dict['msuffix'] if user_dict and user_dict.get('msuffix') else "✘"
        remname = user_dict['mremname'] if user_dict and user_dict.get('mremname') else "✘"
        ...
        buttxt = "Change/Delete Prefix" if prefix != "✘" else "Set Prefix"
        buttons.sbutton(buttxt, f"userset {user_id} suniversal mprefix mirror")
        buttxt = "Change/Delete Suffix" if suffix != "✘" else "Set Suffix"
        buttons.sbutton(buttxt, f"userset {user_id} suniversal msuffix mirror")
        buttxt = "Change/Delete Remname" if remname != "✘" else "Set Remname"
...
╭ <b>Prefix :</b> {escape(prefix)}
├ <b>Suffix :</b> {suffix}
├ <b>User TD Mode :</b> {usertdstatus}
├ <b>User TeamDrive(s) :</b> {usertds if usertds else usertd}
├ <b>Daily Upload :</b> {dailyup} / {dailytlup} per day
├ <b>Remname :</b> <code>{escape(remname)}</code>
'''
    elif key == 'leech':
        prefix = user_dict['prefix'] if user_dict and user_dict.get('prefix') else "✘"
        suffix = user_dict['suffix'] if user_dict and user_dict.get('suffix') else "✘"
        caption = user_dict['caption'] if user_dict and user_dict.get('caption') else "✘"
        remname = user_dict['remname'] if user_dict and user_dict.get('remname') else "✘"
        cfont = user_dict['cfont'][0] if user_dict and user_dict.get('cfont') else "✘"
        userlog = user_dict['userlog'] if user_dict and user_dict.get('userlog') else "✘"
        ...
        buttxt = "Change/Delete UserLog" if userlog != "✘" else "Set UserLog"
        ...
        buttxt = "Change/Delete Caption" if caption != "✘" else "Set Caption"
        ...
        if cfont != "<b>✘</b>": buttons.sbutton("Remove CapFont", f"userset {user_id} cfont")
...
├ <b>Prefix :</b> {escape(prefix)}
├ <b>Suffix :</b> {suffix}
├ <b>Caption :</b> {escape(caption)}
├ <b>CapFont :</b> {cfont}
├ <b>Leech Split Size :</b> {lsplit}
├ <b>Equal Split :</b> {esplits}
├ <b>Daily Leech :</b> {dailyll} / {dailytlle} per day
├ <b>Remname :</b> <code>{escape(remname)}</code>
'''
    if uplan == "Paid 💸" and key:
...
def edit_user_settings(update, context):
    ...
    if user_id != int(data[1]):
        ...
        buttons.sbutton("🔙", f"userset {user_id} back {data[3]}")
        buttons.sbutton("❌", f"userset {user_id} close", 'footer')
        ...
        if config_dict['PAID_SERVICE'] and user_id != OWNER_ID and not is_sudo(user_id) and not is_paid(user_id):
            query.answer("You haven't Paid to Use this Feature. \n#Buy Paid Service", show_alert=True)
            return
        query.answer()
        if handler_dict.get(user_id):
            handler_dict[user_id] = False
        ...
        buttons.sbutton("🔙", f"userset {user_id} back {data[4]}")
        buttons.sbutton("❌", f"userset {user_id} close", 'footer')
        ...
        buttons.sbutton("🔙", f"userset {user_id} back {data[3]}", "footer")
        buttons.sbutton("❌", f"userset {user_id} close", "footer")
